...
for weight in weights:
    ...
    unspillage_rate = 1 - np.count_nonzero(spillage_array) / len(spillage_array)
...
for i, weight in enumerate(weights):
    plt.plot(metrics, rates_by_weight[i], marker='o', linestyle='', markersize=20, label=f'Weight = {weight}')
...
plt.savefig(f"{file_root}/rates_vs_metrics_line_chart.png")
