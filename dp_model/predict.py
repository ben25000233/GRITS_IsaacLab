...
import time
from dp_model.model import SingleObEncoder, DiffusionPolicy, EMAModel, RotationTransformer
...
class LfD():
    def __init__(self):
        ...
        obs_encoder = SingleObEncoder(self.cfg)
    ...
    def run_model(self, image_list, depth_list, eepose_list, seg_pcd):
        ...
        with torch.no_grad():
            ...
            for i in range(image_list.shape[0]):
                ...
                depth_PIL = depth_list[i].astype(np.float32)[..., 0]
            ...
            ee_rot_6d = torch.tensor(rotation_transformer_forward.forward(eepose_list[:, 3:]))
            ee_pose = torch.tensor(eepose_list[:, :3])
            ee_in = torch.cat((ee_pose, ee_rot_6d), -1) 
            
            ee_in = torch.unsqueeze(torch.tensor(ee_in),0).to('cuda:0', dtype=torch.float32) # [1, 5, 7]
